...
## Phase 0: Project Context Preparation (HARD RULE)

### Rule: Large feature path MUST complete Phase 0

**Mandatory checkpoint:**
- Before entering Stage 3, `docs/project-context.md` MUST exist
- If it doesn't exist, MUST block and launch doc-scanner
- **FORBIDDEN to do design without project context**

**Simplified strategy:**
- ✅ File exists → Use directly (don't check freshness)
- ❌ File doesn't exist → Launch doc-scanner (blocking)

**Rationale:**
- Stage 7 will automatically update project-context.md
- Continuous use of agentic-delivery will naturally keep document current

### Blocking Mode Requirements

**doc-scanner MUST wait synchronously:**
- Main agent dispatches doc-scanner in Phase 0, then **waits for completion**
- Cannot proceed asynchronously with Stage 3 work
- Display progress message to user

### Verification Check (Before Stage 3)
...
def verify_phase_0():
    ...
    assert len(content) > 500, "❌ Project context document too small (may be incomplete)"

    # Check required sections
    required_sections = ["Tech Stack", "Coding Conventions", "Directory Structure"]
    for section in required_sections:
        assert section in content, f"❌ Missing required section: {section}"

    log("✅ Phase 0 verification passed")
    return True

# Call before Stage 3 starts
verify_phase_0()
```
...
**Red flag signals:**
- No project-context.md before Stage 3 starts
- Multiple explore agents searching for "what framework does this project use"
- Design document contains proposals conflicting with project's existing patterns

**Handling:**
1. STOP current stage
2. Force rollback to Phase 0
3. Report to user: "Detected missing project context, supplementing..."

## Stage 7: Project Context Update (HARD RULE)

### Rule: Detect and update project context changes

**Mandatory check signals:**
1. New top-level directories added (depth ≤ 2)
2. Major dependencies added to package.json (frameworks, utility libraries)
3. New architectural patterns introduced in design-spec.md
4. New code conventions clarified during review process

**Update strategy:**
- Changes detected → Update project-context.md (append or modify)
- No changes → Leave unchanged
- Main agent completes itself (no subagent needed)

**Post-update behavior:**
- Display updated content in summary
- User can view specific changes via git diff
- If update is unreasonable, user can git revert
...
def detect_context_changes():
    """
    Detect changes affecting project context during implementation
    """
    updates = []

    # Signal 1: New top-level directories
    baseline_commit = git("merge-base HEAD main")
    new_dirs = git(f"diff --name-only {baseline_commit} HEAD") \
        .filter(lambda f: f.count("/") <= 2 and f.endswith("/"))

    if new_dirs:
        updates.append(("Directory Structure", format_dirs(new_dirs)))

    # Signal 2: New major dependencies
    package_diff = git(f"diff {baseline_commit} HEAD package.json")
    new_deps = extract_new_dependencies(package_diff)

    if new_deps:
        updates.append(("Tech Stack", format_deps(new_deps)))

    # Signal 3: New architectural patterns
    design_spec = read_file("docs/<project>/<feature>/design-spec.md")
    patterns = extract_patterns(design_spec)

    if patterns:
        updates.append(("Architecture", format_patterns(patterns)))

    # Signal 4: New code conventions
    review_reports = collect_review_reports()
    conventions = extract_conventions(review_reports)

    if conventions:
        updates.append(("Coding Conventions", format_conventions(conventions)))
...
**Each Stage 7 MUST record:**
- [ ] Whether project context changes were detected
- [ ] If there were changes, which sections were updated
- [ ] Whether update content is visible to user (in summary)

**Red flag signals:**
- Obvious architectural changes but project-context.md not updated
- New tech stack introduced but not recorded
- Important directories added but not synchronized
...
assert file_exists("docs/<project>/<feature>/implementation-tracker.md"), "No plan file"
...
- [ ] Phase 0: project-context.md verification (exists or generated)
- [ ] Stage 2: project-context.md generated by doc-scanner subagent (if needed)
- [ ] Stage 3: design-spec.md written and spec review passed
- [ ] Stage 4: implementation-tracker.md written
...
- [ ] Stage 7: Project context update check performed

**Red flag in log:**
- Stage 3 starts without Phase 0 verification
...
- Obvious changes (new directories, new dependencies) but project-context.md not updated
